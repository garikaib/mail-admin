...
function CountrySelector({ value, onChange, geoRegions = [] }) {
  ...
  const handleToggleRegion = (regionName, countriesStr) => {
    ...
    let updated;
    if (hasAll) {
      updated = currentCodes.filter(c => !regionCodes.includes(c));
    } else {
      updated = Array.from(new Set([...currentCodes, ...regionCodes]));
    }
    onChange(updated.join(', '));
  };
          ...
          {geoRegions.map(reg => {
            ...
            return (
              <button
                key={reg.name}
                type="button"
                onClick={() => handleToggleRegion(reg.name, reg.countries)}
...
function RegionSelector({ value, onChange, geoRegions = [] }) {
  const currentRegions = value ? value.split(',').map(r => r.trim().toUpperCase()).filter(Boolean) : [];

  const handleToggleRegion = (name) => {
    let updated;
    const upperName = name.toUpperCase();
    if (currentRegions.includes(upperName)) {
      updated = currentRegions.filter(r => r !== upperName);
    } else {
      updated = [...currentRegions, upperName];
    }
    onChange(updated.join(', '));
  };
                ...
                <div className="space-y-6">
                  <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-slate-400 text-sm mt-1">Audit trail of critical modifications completed via the Admin Console.</p>
                    </div>
