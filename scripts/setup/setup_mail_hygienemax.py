#!/usr/bin/env python3
"""Setup script for new domain hygienemax.co.zw using MariaDB CLI."""
import os, subprocess, secrets, string

VMAIL_BASE, DOMAIN = "/var/vmail", "hygienemax.co.zw"
USERS = ["director1", "director2", "sales", "marketing", "accounts"]
...
def main():
    ...
    if not domain_id:
        subprocess.run(["sudo", "mariadb", "-e", f"INSERT INTO mailserver.domains (name) VALUES ('{DOMAIN}')"], check=True)
        domain_id = run_sql(f"SELECT id FROM mailserver.domains WHERE name='{DOMAIN}'")
