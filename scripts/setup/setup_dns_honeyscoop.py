#!/usr/bin/env python3
"""Configure Cloudflare DNS for honeyscoop.co.zw."""
import requests

ZONE_ID = "9948b0f9a5b77d81a57e9d6ed63f1956"
API_KEY = "c387a52124c3ece44c4c4e36a2964a152e86a"
EMAIL = "gbdzoma@gmail.com"
HEADERS = {"X-Auth-Email": EMAIL, "X-Auth-Key": API_KEY, "Content-Type": "application/json"}
BASE_URL = f"https://api.cloudflare.com/client/v4/zones/{ZONE_ID}/dns_records"

SERVER_IPV4 = "51.77.222.232"
SERVER_IPV6 = "2001:41d0:305:2100::8406"

# Common DKIM for this server
DKIM_PUBLIC = "v=DKIM1; k=rsa; p="

def get_existing_records():
    r = requests.get(BASE_URL, headers=HEADERS).json()
    if r.get("success"):
        return r["result"]
    return []

def delete_record(record_id):
    requests.delete(f"{BASE_URL}/{record_id}", headers=HEADERS)

def create_record(data):
    return requests.post(BASE_URL, headers=HEADERS, json=data).json()

def main():
    print("Configuring DNS for honeyscoop.co.zw...")
    
    existing = get_existing_records()
    print(f"Found {len(existing)} existing records. Deleting them to start fresh (as requested)...")
    for rec in existing:
        delete_record(rec["id"])
        print(f"  - Deleted {rec['type']} {rec['name']}")

    records = [
        # 1. Wildcard A record
        {"type": "A", "name": "*", "content": SERVER_IPV4, "proxied": False},
        
        # 2. Main A record
        {"type": "A", "name": "honeyscoop.co.zw", "content": SERVER_IPV4, "proxied": False},
        
        # 3. MX Record
        {"type": "MX", "name": "honeyscoop.co.zw", "content": "mail.zimprices.co.zw", "priority": 10},
        
        # 4. Webmail CNAME (proxied)
        {"type": "CNAME", "name": "webmail", "content": "mail.zimprices.co.zw", "proxied": True},
        
        # 5. SPF Record
        {"type": "TXT", "name": "honeyscoop.co.zw", "content": f"v=spf1 mx ip4:{SERVER_IPV4} ip6:{SERVER_IPV6} ~all"},
        
        # 6. DKIM Record
        {"type": "TXT", "name": "mail._domainkey", "content": DKIM_PUBLIC},
        
        # 7. DMARC Record
        {"type": "TXT", "name": "_dmarc", "content": "v=DMARC1; p=quarantine; rua=mailto:dmarc@honeyscoop.co.zw"}
    ]

    for data in records:
        r = create_record(data)
        if r.get("success"):
            print(f" ✓ Created {data['type']} {data['name']}")
        else:
            print(f" ✗ Failed {data['type']} {data['name']}: {r.get('errors')}")

    print("\n✅ DNS configuration complete!")

if __name__ == "__main__":
    main()
