...
def send_email(html_content):
    ...
    msg["Subject"] = SUBJECT
    msg["From"] = "reports@zimprices.co.zw"
    ...
    password = secrets.get("REPORTS_EMAIL_PASSWORD")

    try:
        if password:
            # Send via local Postfix submission port (587) with authentication
            with smtplib.SMTP("localhost", 587) as s:
                s.starttls()
                s.login("reports@zimprices.co.zw", password)
                s.send_message(msg)
        else:
            # Fallback to local unauthenticated port 25
